            ...
            row.forEach(function (col) {
                ...
                for (let styleKey in col.style) {
                    switch (styleKey) {
                        ...
                        case 'width':
                            if (!ws['!cols']) ws['!cols'] = []
                            if (typeof col.style[styleKey] === 'number')
                                ws['!cols'][cur_ci] = { wpx: col.style[styleKey] }
                            else if (col.style[styleKey] === 'auto') ws['!cols'][cur_ci] = { auto: 1 }
                            break
                        case 'height':
                            if (!ws['!rows']) ws['!rows'] = []
                            if (typeof col.style[styleKey] === 'number')
                                ws['!rows'][cur_ri] = { hpx: col.style[styleKey] }
                        ...
                        case 'fontSize':
                            ...
                            break
                        case 'underline':
                            ws[cr].s['underline'] = col.style[styleKey]
                            break
                        case 'color':
                            // ws[cr].s.color['rgb'] = col.style[styleKey]
                            ws[cr].s.color = { theme: 1, rgb: col.style[styleKey].replace('#', '') }

                            console.log('ws[cr].s', ws[cr].s, col.style[styleKey])
                            console.log('cr', cr)
                            console.log('ws', ws)

                            break
                        case 'fontFamily':
                            ws[cr].s.name = col.style[styleKey]
                            break
                        case 'fontWeight':

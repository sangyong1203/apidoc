                ...
                <el-table :data="ruleForm.request"  style="min-height: 160px;" height="100%">
                    ...
                    <el-table-column width="100" >
                        <template #default="scope">
                            <el-button-group >
                                <el-button text icon="delete" type="danger" @click="deleteRequestParam(scope)"/>
                                <el-button text icon="plus" type="info" @click="addSubRequestParam(scope)" v-show="(scope.row.type === 'object array' || scope.row.type === 'object') && scope.row.level < 3"/>
                            </el-button-group>
                        </template>
                    </el-table-column>
                </el-table>
            ...
            <div style="display: flex; flex-direction: column; width: 100%; gap: 8px">
                <el-table :data="ruleForm.response" style="min-height: 160px;" height="100%" >
                    ...
                    <el-table-column width="100">
                        <template #default="scope">
                            <el-button-group>
                                <el-button text icon="delete" type="danger" @click="deleteResponseVar(scope)"/>
                                <el-button text icon="plus" type="info" @click="addSubResponseVar(scope)" v-show="(scope.row.type === 'object array' || scope.row.type === 'object') && scope.row.level < 4"/>
                            </el-button-group>
                        </template>
                    </el-table-column>
                </el-table>
...
import { Message, Notification } from '@/common/dialog/ComDialog'
...
import { makeApiExcelAndDownload } from './ApiExcelDownload.ts'
import type { PostApiModel, RequestType } from './model_service/Type.ts'
